...
function detailHtml(slug, championId) {
  const safeSlug = escapeAttr(slug);
  const safeChampionId = escapeAttr(championId || "Brand");
  return `<!doctype html>
<html lang="zh-CN">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>海克斯大乱斗攻略 - Hex Brawl Helper</title>
    <meta name="description" content="海克斯大乱斗英雄出装和海克斯推荐。" />
    <style>
      :root {
        color-scheme: dark;
        --bg: #18130f;
      ...
      * { box-sizing: border-box; }
      ...
      .back { display: inline-flex; align-items: center; gap: 8px; color: var(--muted); text-decoration: none; font-size: 14px; }
  ...
  <body>
    <main class="shell">
      ...
      <section class="hero">
        <div class="champion-icon">
          <img src="https://ddragon.leagueoflegends.com/cdn/16.13.1/img/champion/${safeChampionId}.png" alt="" />
        </div>
        <div>
          <div class="eyebrow">海克斯大乱斗 · Patch 16.13</div>
      ...
      <section id="builds" class="section">
        <div class="section-head">
          ...
          <div id="buildHint" class="hint">按一图流拆分不同玩法，审核后再精修。</div>
        </div>
      ...
      <footer id="footerText" class="footer">非 Riot 官方项目，当前资料用于海克斯大乱斗一图流展示和人工审核。</footer>
    </main>
    ...
    <script>
      ...
      const copy = {
        zh: {
          back: "← 首页", buildTab: "出装", hexTab: "海克斯",
          buildTitle: "出装路线", buildHint: "按一图流拆分不同玩法，审核后再精修。",
          hexTitle: "海克斯推荐", hexHint: "优先展示当前路线匹配的海克斯；没有单独配置时使用该英雄通用推荐。",
          silver: "白银阶", gold: "黄金阶", prismatic: "棱彩阶",
          route: "路线", score: "评价", noBuilds: "这个英雄的结构化出装还没有录入。", noHexes: "没有可展示的海克斯数据。",
          footer: "非 Riot 官方项目。装备来自一图流人工录入，海克斯来自掌盟截图和一图流交叉整理。", loadError: "数据文件加载失败，请用本地服务器打开页面。"
        },
        en: {
          back: "← Home", buildTab: "Builds", hexTab: "Hexes",
          buildTitle: "Build Routes", buildHint: "Routes are split from the guide images and can be adjusted after review.",
          hexTitle: "Hex Recommendations", hexHint: "Shows route-specific hexes first; falls back to general recommendations when needed.",
          silver: "Silver", gold: "Gold", prismatic: "Prismatic",
          route: "Route", score: "Note", noBuilds: "Structured item builds are not entered yet.", noHexes: "No hex data is available yet.",
          footer: "Unofficial Riot fan project. Items are manually entered from guide images; hexes are cross-checked from app captures and guide images.", loadError: "Failed to load the data file. Open this page through a local server."
        },
        ja: {
          back: "← ホーム", buildTab: "ビルド", hexTab: "ヘクス",
          buildTitle: "ビルドルート", buildHint: "ガイド画像をもとにルート別に整理し、確認後に調整します。",
          hexTitle: "ヘクス推奨", hexHint: "ルート別のヘクスを優先表示し、未設定の場合は共通推奨を表示します。",
          silver: "シルバー", gold: "ゴールド", prismatic: "プリズム",
          route: "ルート", score: "評価", noBuilds: "このチャンピオンの構造化ビルドは未入力です。", noHexes: "表示できるヘクスデータがありません。",
          footer: "Riot 非公式ファンプロジェクト。装備はガイド画像から手入力し、ヘクスはアプリの取得画像とガイド画像を照合しています。", loadError: "データファイルを読み込めません。ローカルサーバー経由で開いてください。"
        },
        ko: {
          back: "← 홈", buildTab: "빌드", hexTab: "증강",
          buildTitle: "빌드 경로", buildHint: "가이드 이미지를 기준으로 경로를 나누고 검수 후 조정합니다.",
          hexTitle: "증강 추천", hexHint: "경로별 증강을 우선 표시하고, 없으면 공통 추천을 표시합니다.",
          silver: "실버", gold: "골드", prismatic: "프리즘",
          route: "경로", score: "평가", noBuilds: "이 챔피언의 구조화 빌드가 아직 입력되지 않았습니다.", noHexes: "표시할 증강 데이터가 없습니다.",
          footer: "Riot 비공식 팬 프로젝트입니다. 아이템은 가이드 이미지에서 수동 입력하고, 증강은 앱 캡처와 가이드 이미지를 교차 확인합니다.", loadError: "데이터 파일을 불러오지 못했습니다. 로컬 서버로 열어 주세요."
        }
      };
      ...
      function hexIcon(path) { return cdragonBase + path; }
      ...
      function renderText() {
        const c = t();
        const dataCopy = localizedData();
        const championName = championData?.champion?.names?.[state.lang] || championData?.champion?.names?.en || championData?.champion?.names?.zh || "";
        elements.language.value = state.lang;
        document.documentElement.lang = state.lang === "zh" ? "zh-CN" : state.lang === "ja" ? "ja" : state.lang === "ko" ? "ko" : "en";
        document.title = championName ? championName + " 海克斯大乱斗攻略 - Hex Brawl Helper" : "海克斯大乱斗攻略 - Hex Brawl Helper";
...
for (const file of files) {
  ...
  await writeFile(path.join(pageDir, "index.html"), detailHtml(slug, data.champion?.id), "utf8");
  updated += 1;
}

console.log(`Updated ${updated} Hex Brawl detail pages.`);
