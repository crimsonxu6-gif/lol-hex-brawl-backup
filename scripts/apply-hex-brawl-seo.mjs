...
<html lang="zh-CN">
  ...
  <body>
    ...
    <script>
      ...
      const languageStorageKey = "hexBrawlLanguage";
      const supportedLanguages = ["zh", "en", "ja", "ko", "es"];
      const initialLang = new URLSearchParams(window.location.search).get("lang");
      function storedLanguage() {
        try { return localStorage.getItem(languageStorageKey); } catch (error) { return null; }
      }
      function saveLanguage(lang) {
        try { localStorage.setItem(languageStorageKey, lang); } catch (error) {}
      }
      function documentLang(lang) {
        return lang === "zh" ? "zh-CN" : lang === "ja" ? "ja" : lang === "ko" ? "ko" : lang === "es" ? "es" : "en";
      }
      const state = {
        lang: supportedLanguages.includes(initialLang) ? initialLang : (supportedLanguages.includes(storedLanguage()) ? storedLanguage() : "zh"),
        selectedBuild: null
      };
      ...
      const copy = {
        ...
        es: {
          back: "Inicio", buildTab: "Builds", hexTab: "Aumentos",
          buildTitle: "Rutas de build", buildHint: "Las rutas de build se organizan a partir de guias e informacion actual de objetos.",
          hexTitle: "Mejores aumentos", hexHint: "Las recomendaciones se agrupan por Plata, Oro y Prismatico.",
          silver: "Plata", gold: "Oro", prismatic: "Prismatico",
          route: "Ruta", score: "Nota", noBuilds: "Aun no hay builds estructuradas para este campeon.", noHexes: "Aun no hay datos de aumentos disponibles.",
          footer: "Proyecto no oficial de fans. Los datos de objetos y aumentos sirven como referencia para Hex Brawl.", loadError: "No se pudo cargar el archivo de datos. Abre esta pagina mediante un servidor local."
        }
      };
      ...
      function renderText() {
        const c = t();
        const dataCopy = localizedData();
        const championName = championData?.champion?.names?.[state.lang] || championData?.champion?.names?.en || championData?.seo?.name || "";
        elements.language.value = state.lang;
        document.documentElement.lang = documentLang(state.lang);
